...
### 4.2 ポインティングの情報理論的定式化

ポインティング行為による予測誤差圧縮を情報理論的に定式化する。

潜在変数 $Z$（ユーザーが操作したい対象のカテゴリ）、言語入力 $X$、ポインティング信号 $P$ とする。

**ポインティングなし（言語のみ）の場合：**

$$H(Z | X)$$

言語入力のみでは、$Z$ の事後分布 $p(z|x)$ は多峰的（multi-modal）となる。例えば「ボタンを赤くして」という指示に対して、ページ上の複数のボタンが候補となり、エントロピーは高い。

**ポインティングありの場合：**

$$H(Z | X, P)$$

ポインティング信号 $P$ の付加により、$p(z|x,p)$ は単峰的（uni-modal）に拘束される。特定のDOM要素が直示的に指定されることで、参照の曖昧性が解消される。

**予測誤差圧縮量：**

$$\Delta H = H(Z | X) - H(Z | X, P) > 0$$

この $\Delta H$ がポインティングによる予測誤差圧縮量であり、本研究の中心的主張は、この値が有意に正であるということである。

等価的に、ポインティングの情報的寄与は $X$ と $P$ の間の条件付き相互情報量として表現できる：

$$I(Z; P | X) = H(Z | X) - H(Z | X, P) = \Delta H$$
...
ポインティングは $F_{interaction}$（相互作用に起因する予測誤差）を急激に減少させる操作であり、言語的交渉のみの場合に比べて収束を劇的に加速する。これが、siegeNginにおけるセッション内マイクロ発達が従来のロボティクスCPCより高速である理論的根拠となる。
